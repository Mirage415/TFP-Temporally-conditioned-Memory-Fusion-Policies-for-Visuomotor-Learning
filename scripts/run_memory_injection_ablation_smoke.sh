...
mkdir -p outputs/memory_ablation/smoke

for config in "${CONFIGS[@]}"; do
  uv run scripts/train.py "$config" \
    --exp-name="smoke_${config}" \
    --checkpoint-base-dir=outputs/memory_ablation/smoke/checkpoints \
    --assets-base-dir="${OPENPI_MEMORY_ABLATION_ASSETS_DIR}" \
    --temporal-cache-dir="${OPENPI_MEMORY_ABLATION_TEMPORAL_CACHE_DIR}" \
    --num-train-steps=1 \
    --batch-size="${OPENPI_MEMORY_ABLATION_BATCH_SIZE:-1}" \
    --fsdp-devices=1 \
    --save-interval=1 \
    --log-interval=1 \
    --no-wandb-enabled \
